...
#'   `iss_average_sector_emission_intensities` object
#'
#' @export

prepare_iss_average_sector_emission_intensities <- function(iss_company_emissions,
  ...
  factset_entity_info <-
    factset_entity_info %>%
    dplyr::select("factset_entity_id", "iso_country", "sector_code", "factset_sector_desc") %>%
  ...
  iss_entity_emission_intensities %>%
    dplyr::inner_join(factset_entity_info, by = "factset_entity_id") %>%
    dplyr::summarise(
      emission_intensity_per_mkt_val = stats::weighted.mean(
        .data[["emission_intensity_per_mkt_val"]],
        .data[["ff_mkt_val"]],
        na.rm = TRUE
      ),
      emission_intensity_per_debt = stats::weighted.mean(
